...
## Coding & Practices
6. **Core stack**: Python, HTML/CSS, JS/TS, React, Django, Flask, TypeScript, Tailwind, HTTP requests, APIs, YAML/JSON, Firebase, Markdown, Swisft for iOS native app development
...
## Security Requirements (Firebase Stack)
Tech: React/Vite → Firebase Hosting, Cloud Functions, Firestore, Auth, Storage, **Google Cloud Secret Manager**

CRITICAL RULES:
1. **NO secrets in frontend code.** NO `VITE_` variables for API keys or any secret material.
2. **ALL external API calls** (OpenAI, AWS, Stripe, etc.) go through **Cloud Functions**.
3. **Store secrets in Google Cloud Secret Manager by default.**
   - For Firebase/Cloud Functions, use `defineSecret()` / Secrets Manager integration (backed by Secret Manager).
   - **Do not store secrets in `.env` files** (local or repo). Use Secret Manager access for local dev as well.
4. **Never include secrets in plaintext** (logs, commit history, plaintext files, tickets, screenshots). If a secret is exposed, rotate it and purge history immediately.
5. **Verify authentication server-side** in Cloud Functions using `request.auth`.
6. **Frontend calls Cloud Functions via `httpsCallable()`**, never external APIs directly.

If generating code that calls an external API, ALWAYS:
- Put the API call in a Cloud Function
- Store the key in Secret Manager
- Have the frontend call the Cloud Function
...
- **No secrets in frontend**; no `VITE_` API keys.
- **No secrets in `.env` files**; store secrets in **Google Cloud Secret Manager** (use `defineSecret()` for Cloud Functions).
- **External APIs only via Cloud Functions**; frontend uses `httpsCallable()`.
- **Never log or persist secrets in plaintext** (logs, files, tickets, screenshots, commit history).
