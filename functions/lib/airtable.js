...
function processStudents(records, baseConfig, targetYear, studentTruthLookup) {
    ...
    for (const record of records) {
        try {
            ...
            if (studentTruthLookup && studentTruthLookup.size > 0) {
                ...
                const lookupKey = `${(0, types_1.normalizeString)(displayName).replace(/\s+/g, ' ')}|${targetYear}`;
            ...
            students.set(studentKey, {
                ...
                campusKey: effectiveCampus ? (0, types_1.createCampusKey)(effectiveCampus, finalMcLeader) : '',
    ...
    const singleNameLeaders = new Set();
    for (const student of students.values()) {
        if (student.mcLeader && !student.mcLeader.includes(',') &&
            student.campus?.toLowerCase().includes('micro')) {
            singleNameLeaders.add((0, types_1.normalizeString)(student.mcLeader));
        }
    }
    ...
    for (const [key, student] of students) {
        if (!student.mcLeader || !student.mcLeader.includes(','))
            continue;
        if (!student.campus?.toLowerCase().includes('micro'))
            continue;
        ...
        for (const name of names) {
            if (singleNameLeaders.has((0, types_1.normalizeString)(name))) {
                matchedLeader = name;
                break;
            }
        }
        ...
        student.campusKey = student.campus
            ? (0, types_1.createCampusKey)(student.campus, matchedLeader)
            : '';
...
async function syncAirtableData(db, token, targetSchoolYear) {
    ...
    const studentTruthLookups = new Map();
    // Process each base
        ...
        try {
            ...
            if (base.tables.studentTruth) {
                ...
                for (const record of truthRecords) {
                    ...
                    if (truthMcLeader && (truthMcLeader.toLowerCase().includes('sandbox') ||
                    ...
                    const normalizedName = nameParts.length >= 2
                        ? (0, types_1.normalizeString)(`${nameParts[1]} ${nameParts[0]}`).replace(/\s+/g, ' ') // "First Last"
                        : (0, types_1.normalizeString)(cleanName).replace(/\s+/g, ' ');
    ...
    for (const [year, students] of allStudentsByYear) {
        ...
        for (const [key, student] of students) {
            ...
            const truthKey = `${(0, types_1.normalizeString)(displayName).replace(/\s+/g, ' ')}|${year}`;
            ...
            if (truthLookupForYear) {
                ...
                if (truthEntry) {
                    ...
                    if (totalPossible > 0 && totalPresent <= 0) {
                        ...
                        if ((0, types_1.isNonStarterStatus)(status) || isWithdrawal) {
                            attendedAtLeastOnce = false;
                        }
                    }
                }
                    ...
                    if ((0, types_1.isNonStarterStatus)(status)) {
                        attendedAtLeastOnce = false;
                    }
                }
            }
